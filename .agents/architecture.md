...
- Android may create an Always-on `VpnService` through `onStartCommand()` without Clash Arc's bound-service path. The service
...
- macOS and Linux build a standalone `Clash ArcCore` process used by the desktop socket integration.
- Windows builds `Clash ArcCore.exe`, the Rust `Clash ArcHelperService.exe` privileged helper, and a
...
FFI library, while setup is only the build and packaging bridge for Clash Arc's external core artifacts.
...
- `/ping` is loopback-only and requires no request token. The Helper compares the requested SHA256 with its embedded value
  and checks that the fixed `Clash ArcCore.exe` beside it exists; `/start` performs the actual Core SHA256 verification before
...
- Flutter creates a 128-bit lowercase-hex session ID and uses it as the random named-pipe suffix. `/start` receives only
  that address and session ID, validates the fixed `Clash ArcCore_<session>` namespace, starts the fixed Core beside the
...
Architecture detection is automatic. The `--description` flag passed to `flutter_distributor` adds arch suffixes to artifact names, such as `Clash Arc-0.8.93-macos-arm64.dmg`.
...
- `Clash ArcHelperService.exe install` stops and removes any stale registration, creates the auto-start service for the
  current executable path, starts it, and waits for the running state.
- `Clash ArcHelperService.exe uninstall` stops the service, waits for shutdown, removes its registration, and is also used
...
- `GET /ping?coreSha256=...` returns the current Helper executable path with `x-clasharc-helper-protocol` when the
