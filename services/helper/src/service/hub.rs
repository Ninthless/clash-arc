...
const CORE_PIPE_PREFIX: &str = r"\\.\pipe\Clash ArcCore_";
const PROTOCOL_VERSION_HEADER: &str = "x-clasharc-helper-protocol";
...
mod tests {
    ...
    async fn ping_returns_running_helper_path_for_verified_core() {
        let response = ping_response(Ok(PathBuf::from("Clash ArcHelperService.exe")));
    ...
    async fn start_rejects_a_caller_supplied_core_argument() {
        let response = warp::test::request()
            ...
            .body(
                r#"{"address":"\\\\.\\pipe\\Clash ArcCore_0123456789abcdef0123456789abcdef","sessionId":"0123456789abcdef0123456789abcdef","path":"attacker.exe"}"#,
            )
            .reply(&routes())
            .await;
    ...
    async fn start_releases_the_managed_core_before_rejecting_an_unverified_core() {
        ...
        *MANAGED_CORE.lock().unwrap() = Some(ManagedCore {
        ...
        let response = warp::test::request()
            ...
            .path("/start")
            .json(&StartParams {
                address: r"\\.\pipe\Clash ArcCore_0123456789abcdef0123456789abcdef".to_string(),
    ...
    fn only_accepts_random_core_pipe_namespace() {
        assert!(is_allowed_core_pipe(
            r"\\.\pipe\Clash ArcCore_0123456789abcdef0123456789abcdef"
        ));
        assert!(!is_allowed_core_pipe(r"\\.\pipe\Clash ArcCore"));
        assert!(!is_allowed_core_pipe(
            r"\\.\pipe\Other_0123456789abcdef0123456789abcdef"
        ));
        assert!(!is_allowed_core_pipe(
            r"\\.\pipe\Clash ArcCore_0123456789abcdef"
        ));
        assert!(!is_allowed_core_pipe(
            r"\\.\pipe\Clash ArcCore_0123456789abcdef0123456789abcdeg"
        ));
        assert!(!is_allowed_core_pipe(
            r"\\.\pipe\Clash ArcCore_ABCDEF0123456789abcdef0123456789"
        ));
    }
}
